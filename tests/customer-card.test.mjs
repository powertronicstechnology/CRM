...
test('stage controls save independently, preserve failures and keep finance permissions',async()=>{
 const dir=await fs.mkdtemp(new URL('./.card-',import.meta.url));let view;
 try{
 ...
 await act(async()=>save.props.onClick());assert.equal(writes,1);assert.equal(view.root.findAllByProps({role:'alert'}).length,1);assert.equal(select.props.value,'DOCUMENTS PENDING');
 fail=false;await act(async()=>save.props.onClick());assert.equal(writes,2);assert.equal(opened,1);assert.equal(view.root.findAllByProps({role:'alert'}).length,0);
 await act(async()=>view.update(React.createElement(Card,{...props,canSeeFinance:false,customer:{...customer,stage:'DOCUMENTS PENDING'}})));
 ...
 }finally{if(view)await act(async()=>view.unmount());await fs.rm(dir,{recursive:true,force:true});}
});
