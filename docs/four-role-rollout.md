...
- Excel export downloads one .xlsx workbook with Customers and Financial worksheets for Admin/Manager, Customers only for Staff, and Financial only for Accounts. It exports active records across stages for the selected month. The customer sheet follows the requested column order, keeps the two named placeholder columns blank, and appends Subdivision, CRN and record ID. Unnamed empty trailing spreadsheet columns are omitted. Remaining operational fields accompany Financial for Admin/Manager only. Values are exported as stored; quotation variants are not reconciled. The exporter is covered by workbook round-trip tests. No Supabase change is required for this export feature.

Export Records page: choose All (both worksheets), Customers, or Finance, limited by role. Choose all active records or the header month filter. The preview shows the first ten matching records with the same columns and values as the downloaded workbook. Add Lead now uses a wider two-column laptop form.
