...
export default function OperationsView({ user }) {
    ...
    const rename = async () => {
        if (lock.current || !renaming || !newName.trim()) return;
        lock.current = true; setBusy(true); setError(''); setMessage('');
        try {
            const { data, error } = await supabase.rpc('rename_dropdown_option', {p_category: category, p_old: renaming, p_new: newName.trim()});
            if (error) throw error;
            setRows(previous => previous.map(row => row.category === category && row.label === renaming ? {...row, label: newName.trim()} : row));
            setMessage(`Renamed to ${newName.trim()}. Updated ${data.updated} customer records.`);
            setRenaming(null); setSearch('');
            window.dispatchEvent(new Event('metadata-updated'));
            window.dispatchEvent(new Event('customer-records-updated'));
        } catch(e) { setError(e.code === 'PGRST202' ? 'Run the rename dropdown SQL patch in Supabase, then try again.' : e.message || 'Rename failed. Please try again.'); }
        finally { lock.current = false; setBusy(false); }
    };
    return <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="grid md:grid-cols-[230px_1fr] gap-5">
            <nav aria-label="Dropdown categories" className="space-y-1">{Object.entries(DROPDOWN_LABELS).map(([key,label]) => <button key={key} disabled={busy} onClick={() => {setCategory(key);setRenaming(null);setSearch('');setValue('');setRemoving(null);setError('');setMessage('');}} className={`w-full text-left px-4 py-3 rounded-xl text-sm ${category === key ? 'bg-stone-900 text-white' : 'text-stone-600 hover:bg-stone-100'}`}>{label}<span className="float-right opacity-60">{meta[key].length}</span></button>)}</nav>
            <section className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4">
                ...
                <ul className="divide-y divide-stone-100">{options.map(label => <li key={label} className="py-3 flex items-center justify-between gap-3 text-sm"><span className="break-words">{label}</span><div className="flex gap-1"><button aria-label={`Rename ${label}`} disabled={busy || !meta._ready} onClick={() => {setRenaming(label);setNewName(label);setRemoving(null);setError('');}} className="p-2 text-stone-500 hover:bg-stone-100 rounded-lg"><Pencil size={16}/></button><button aria-label={`Remove ${label}`} disabled={busy || !meta._ready} onClick={() => {setRemoving(label);setRenaming(null);setError('');}} className="p-2 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-lg disabled:opacity-30"><Trash2 size={16}/></button></div></li>)}</ul>
                {!options.length && <p className="text-sm text-stone-500">No options found.</p>}
                {renaming && <form onSubmit={e => {e.preventDefault();rename();}} className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3 text-sm">
                    <p>Rename <strong>{renaming}</strong></p>
                    <input aria-label="New name" autoFocus maxLength={120} value={newName} disabled={busy} onChange={e => setNewName(e.target.value)} className="w-full rounded-lg border border-stone-200 px-3 py-2"/>
                    <p className="text-stone-600">This updates the dropdown and all matching customer records, including records in Trash.</p>
                    <div className="flex gap-2"><button type="button" disabled={busy} onClick={() => setRenaming(null)} className="px-3 py-2 rounded-lg border border-stone-300">Cancel</button><button disabled={busy || !newName.trim() || newName.trim() === renaming} className="px-3 py-2 rounded-lg bg-stone-900 text-white disabled:opacity-40">{busy ? 'Saving…' : 'Rename and update records'}</button></div>
                </form>}
